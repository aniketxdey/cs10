 ...
 */
public class ImageProcessingGUI extends WanderingPulsar.DrawingGUI {
	private int radius = 5;				// how big the drawing effects are

	private boolean brushUp = true;

	private WanderingPulsar.ImageProcessor proc;		// handles the image processing
	 ...
	 */
	public ImageProcessingGUI(WanderingPulsar.ImageProcessor proc) {
		super("Image processing", proc.getImage().getWidth(), proc.getImage().getHeight());
		this.proc = proc;
	}

	/**
	 * WanderingPulsar.DrawingGUI method, here showing the current image
	 */
	@Override
	...
	/**
	 * WanderingPulsar.DrawingGUI method, here dispatching on image processing operations
	...
	/**
	 * WanderingPulsar.DrawingGUI method, here manipulating the image at/near the mouse location
	 */
	@Override
	...
	public void handleMouseMotion(int x, int y) {
		if (!brushUp) {
			proc.drawCircle(x, y, radius, pickedColor);
		}
  		repaint();
	}
	...
	public static void main(String[] args) { 
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				new ImageProcessingGUI(new WanderingPulsar.ImageProcessor(baker));
			}
		});
	}
}
