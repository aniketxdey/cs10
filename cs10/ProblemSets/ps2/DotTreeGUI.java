...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			Dot d =  new Dot(x, y);
			if (tree == null) {
				tree = new PointQuadtree<>(d, 0, 0, width, height);
			}
			else {
				tree.insert(d);
			}
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0, 0, width, height); // central point
		tree.insert(new Dot(600,500));
		tree.insert(new Dot(200,100));
		tree.insert(new Dot(700,200));
		int bad = 0;
		bad += testFind(600,500,10,3,3,1);
		bad += testFind(200,100,100,3,2,1);
		bad += testFind(700,200,150,3,3,2);
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		Dot dot = tree.getPoint();
		g.fillOval((int)dot.getX() - dotRadius, (int)dot.getY() - dotRadius, 2 * dotRadius, 2 * dotRadius);
		g.drawLine(tree.getX1(), (int)dot.getY(), tree.getX2(), (int)dot.getY());
		g.drawLine((int)dot.getX(), tree.getY1(), (int)dot.getX(), tree.getY2());
